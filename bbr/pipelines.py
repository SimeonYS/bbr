...
class BbrPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bbr.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bbr`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bbr WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bbr (date, title, link, content)"
